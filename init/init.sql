...
BEGIN
    IF
        ...
        FROM bike b
        WHERE st_distance(ST_SetSRID(b.coordinate::geometry, 4326), ST_SetSRID(NEW.coordinate::geometry, 4326)) <=
              NEW.radius;
...
BEGIN
    IF
        ...
        FROM parking_area p
        WHERE st_distance(ST_SetSRID(p.coordinate::geometry, 4326), ST_SetSRID(NEW.coordinate::geometry, 4326)) <=
              p.radius;
